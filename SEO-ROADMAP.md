...
**Data range analyzed:** GSC unavailable this cycle (MCP server would not connect, 2nd cycle in a row), 2026-08-09→08-11 (Clarity 3d), 30d+lifetime (funnel/sales)
...
- **Flagged, not touched (2026-08-11) — fabricated social-proof numbers already live on
  event pages, conflicts with this routine's own "never fabricate data" rule.**
  `src/components/UrgencyComponents.tsx`'s `PeopleWatching` (`{count} אנשים צופים באירוע זה
  כרגע`) and `RecentPurchaseToast` (`{name} רכש/ה כרטיס {time}`) both generate their numbers/
  names from `Math.random()`, not real traffic or sales data — pre-existing code, not added by
  any `/seo-update` cycle. This is out of scope for an unattended run (no GSC signal behind it,
  and swapping to real data needs `realBuyerCount`/`realSalesPerDate` wired into the page at
  render time, a real implementation task, not a copy tweak) — surfacing here for the site
  owner to decide: replace with real per-party numbers (available via the sales API, would be
  legitimately compelling since real buyer counts do exist for some parties), or remove
  entirely if the legal/trust risk of fabricated social proof outweighs the conversion lift.
