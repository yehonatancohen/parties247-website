...
**Data range analyzed:** 2026-06-30 → 2026-07-27 (GSC lag: latest available row is 07-27)
...
| Sitemap Pages Submitted | 374, 0 errors | ↑↑ Up sharply from 189 — expected: archive pages (`/archive/[slug]`) now being emitted alongside upcoming `/event/[slug]` entries, see archive feature note below |
...
- [ ] **Rewrite Jimmy Who & Moon Child title tags** (2026-07-27) — description rewrite (07-17) and NightClub schema (07-24) both failed to move CTR after a full cycle each; titles now lead with concrete practical hooks (street name, VIP table, Happy Hour) instead of generic "כרטיסים ואירועים". 2026-07-30: still 0/25 (jimmy-who) and 0/7 (moon-child) clicks since the 07-27 rewrite, but per GSC lag that's at most ~1 day of real post-change data (same situation as 07-21) — too early to judge, left alone again, decision deferred to the 08-02 cycle.
...
- [x] **`/parties/18-plus-parties-tel-aviv` 69.57% quick-back rate (2026-07-21 flag)** — RESOLVED as a false alarm, 2026-07-30: reviewed 5 session recordings directly. The "quick-back" pattern is users repeatedly navigating between the listing page and individual event pages (clicking "פרטים וכרטיסים"/details-and-tickets, then back), not bouncing off the site — healthy browsing on a page that's already converting well (44 clicks/458 impr, 9.6% CTR, pos 7.0). Retired from monitoring.
- [x] **Fix Haifa/Tel Aviv query cannibalization** (2026-07-30) — same pattern as the 07-06 jimmy-who/moon-child root cause, caught by checking exact-query page-splits GSC's quick-wins detector can't surface (it only flags pos 4–10). Two issues found and fixed: (1) `/parties/parties-in-haifa-and-north` (title "מסיבות בחיפה ובצפון") was competing directly with `/cities/haifa` (rewritten 07-17 to target "מסיבות בחיפה" exactly) for the identical query — both stuck around position ~20-21 with near-0 clicks. Retitled the region page to "מסיבות בצפון – קריות, עכו והגליל" so it no longer contains the literal phrase `/cities/haifa` owns. (2) "מסיבות בתל אביב" (41 impressions, pos 47 overall) was split across 4 pages with none in the top 20: `/cities/tel-aviv` (pos 32), `/all-parties` (pos 49), `/parties/parties-in-tel-aviv-today` (pos 61), and `/carousels/תל-אביב` (pos 71). The last one is a thin, auto-titled carousel page with no unique content beyond a party grid — added `robots: {index: false}` to all `/carousels/[carouselSlug]` pages (they were never meant to be search landing pages, just homepage widgets) to stop them diluting the dedicated city/genre pages. Watch `/cities/haifa` and `/cities/tel-aviv` position next cycle.
- [ ] **`/articles/מדריך-מועדוני-טכנו-בתל-אביב` is a top performer** (noted 2026-07-30) — 8 clicks / 55 impressions / 14.5% CTR / position 6.2, one of the best-converting pages on the site. Validates the unstarted Phase 4 "articles targeting Hebrew party queries" item — worth writing more of these rather than treating Phase 4 as low priority.
...
| Date | What Changed |
|---|---|
| 2026-07-30 | Unattended scheduled run. 28d (06-30→07-27, GSC lag): clicks/impressions holding the up-trend from last cycle (best days 07-23: 48 clicks/684 impr, 07-18: 51 clicks/778 impr), position mostly 8–10, dipping to 10.5 on 07-26. Sitemap 374→416 submitted, 0 errors (continued archive-page growth, expected). Jimmy Who/Moon Child: 0/25 and 0/7 clicks since the 07-27 title rewrite, but that's only ~1 day of real post-change data given GSC lag — same situation as 07-21, too early to judge, **not** deprioritizing yet as previously flagged as a possibility (advisor caught that the roadmap's own "if still flat next cycle" condition means 08-02, not this run). Went looking for cannibalization GSC's quick-wins detector can't surface (it only flags position 4–10) and found two real ones, both fixed: (1) `/parties/parties-in-haifa-and-north` was competing with `/cities/haifa` for the literal phrase "מסיבות בחיפה" — retitled to "מסיבות בצפון – קריות, עכו והגליל"; (2) "מסיבות בתל אביב" (41 impr, pos 47) was split across 4 pages including a thin, auto-titled `/carousels/[carouselSlug]` page with no unique content — added `robots: {index: false}` to all carousel pages so they stop diluting `/cities/tel-aviv` and other dedicated landing pages (they're homepage widgets, not intended search landing pages, and weren't in the sitemap to begin with). Also noted `/articles/מדריך-מועדוני-טכנו-בתל-אביב` (8 clicks, 14.5% CTR, pos 6.2) as one of the best-converting pages on the site — validates the still-unstarted Phase 4 "articles" item. Clarity (last 3 days, 07-28→07-30): reviewed 5 session recordings for `/parties/18-plus-parties-tel-aviv`'s 69.57% quick-back rate (flagged 07-21) — it's users clicking "פרטים וכרטיסים" to event pages and navigating back to browse more events, i.e. healthy engagement, not a bounce problem; retired from monitoring. Homepage/`/all-parties` Clarity metrics not re-pulled this cycle (page-level query focused on cannibalization investigation instead); the uncommitted homepage redesign (`HomeClient`/`HotEventsCarousel`/`SocialsCta`) is still sitting in the local working tree as of this run, untouched. |
