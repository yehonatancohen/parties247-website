...
const CITY_FAQS: Record<string, { question: string; answer: string }[]> = {
  'אילת': [
    {
      question: 'אילו פסטיבלים מתקיימים באילת ב-2026?',
      answer: 'אזור אילת והערבה מארח כמה פסטיבלי טבע וטראנס גדולים לאורך השנה, בהם אמנזיה בחוות הגמלים וריטואל, עם שיא בחגי ראש השנה וסוכות. כל הפסטיבלים הקרובים עם תאריכים וכרטיסים מופיעים ברשימה למעלה.',
    },
    {
      question: 'איפה חוות הגמלים ואיך מגיעים לפסטיבלים באזור?',
      answer: 'חוות הגמלים ממוקמת בערבה, צפונית לאילת, ומשמשת מתחם לפסטיבלי מדבר. רוב ההפקות מפעילות הסעות מאורגנות מהמרכז ומאילת – פרטי ההסעה מופיעים בדף הרכישה של כל אירוע.',
    },
    {
      question: 'מתי כדאי לקנות כרטיסים לאירועים באילת?',
      answer: 'מוקדם ככל האפשר. פסטיבלים באזור אילת נמכרים מראש ולעיתים אוזלים שבועות לפני התאריך, ומחירי ה-Early Bird נגמרים הרבה לפני מחיר הדלת. לאירועי חגים במיוחד מומלץ לשריין כרטיס ברגע שסוגרים תאריך.',
    },
    {
...
const buildCityFaqs = (cityName: string) => CITY_FAQS[cityName] ?? [
  {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="mx-4 max-w-[860px] rounded-[28px] bg-tile p-6 text-ink-2 sm:p-10 md:mx-auto">
        <h2 className="text-[24px] font-bold text-ink sm:text-[28px] mb-4">למה שווה לצאת בעיר</h2>
      ...
      <section className="container mx-auto max-w-4xl px-4">
        <div className="rounded-[28px] bg-tile p-6 sm:p-10">
          <h2 className="text-[24px] font-bold text-ink sm:text-[28px] mb-6">שאלות נפוצות על מסיבות ב{displayCityName}</h2>
