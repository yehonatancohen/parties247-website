...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const section = areaSections.find((item) => item.id === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/party-discovery/area/${section.id}` },
  };
}

export default async function PartyDiscoveryAreaDetail({ params }: Props) {
  const section = areaSections.find((item) => item.id === params.slug);
    ...
    <div className="min-h-screen bg-gradient-to-b from-jungle-deep via-[#0c1713] to-black text-white">
      <div className="container mx-auto px-4 pb-16 pt-14 md:pt-16 space-y-8">
        <div className="flex justify-start">
          <BackButton fallbackHref="/party-discovery/area" label="חזרה" />
        </div>

        <div className="space-y-3">
          <p className="text-xs uppercase tracking-[0.25em] text-emerald-200/70">עמוד אזור</p>
